...
#  save(setLabels,file="setLabels.Rda")
  ...
  write.table(colors_met,file="Tables/Allmetabolite_modules.txt",sep="\t",row.names=FALSE)
  
  if(is.na(sigfeats)==FALSE){
    sub_colors_met<-colors_met[which(data_matrix_orig$mz%in%sigfeats$mz),]
    write.table(sub_colors_met,file="Tables/Sigmetabolite_modules.txt",sep="\t",row.names=FALSE)
  }
