      ...
      if(is.na(all(diff(match(rnames_xmat,rnames_ymat))))==FALSE){
        if(all(diff(match(rnames_xmat,rnames_ymat)) > 0)==TRUE){
          ...
          data_matrix<-data_preprocess(Xmat=Xmat,Ymat=Ymat,feature_table_file=feature_table_file,parentoutput_dir=parentoutput_dir,class_labels_file=NA,num_replicates=num_replicates,feat.filt.thresh=NA,summarize.replicates=summarize.replicates,summary.method=summary.method,
                                       ...
                                       summary.na.replacement=summary.na.replacement,featselmethod=featselmethod,TIC_norm=TIC_norm,normalization.method=normalization.method,input.intensity.scale=input.intensity.scale,log2.transform.constant=log2.transform.constant)
      ...
      if(dim(data_m_fc)[1]>0){
        ...
        if(featselmethod=="limma2way")
        {
          ...
          if(limmarobust==FALSE){
            ...
            filename<-"Tables/limmarobust_2wayanova_posthocresults.txt"
          }
          colnames(data_limma_fdrall_withfeats)<-as.character(cnames_tab)
          ...
          if(featselmethod=="lm1wayanovarepeat"| featselmethod=="lmregrepeat"){
            ...
            if(featselmethod=="lm1wayanovarepeat"){
              ...
              res1<-parApply(cl,data_m_fc,1,function(x,classlabels_response_mat,subject_inf,modeltype){
                ...
                ##save(data_mat_anova,subject_inf,modeltype,file="debug.Rda")
                anova_res<-diffexplmregrepeat(dataA=data_mat_anova,subject_inf=subject_inf,modeltype=modeltype)
      ...
      if(length(goodip)>1){
        ...
        if(analysismode=="classification"){
          ...
          if(analysismode=="regression"){
            ...
            if(num_sig_feats>=3){
              ...
              get_pcascoredistplots(X=goodfeats_temp,Y=classlabels_orig,feature_table_file=NA,parentoutput_dir=getwd(),
        ...
        if(logistic_reg==TRUE){
          ...
          if(poisson_reg==TRUE){
            ...
            fname4<-paste(parentfeatselmethod,".selected.features.final.txt",sep="")
          }
        }
            ...
            if(pairedanalysis==TRUE || timeseries.lineplots==TRUE)
            {
              ...
              #try(
                var_sum_list<-get_lineplots(X=goodfeats_temp,Y=classlabels_orig,feature_table_file=NA,
                                ...
                                lineplot.col.opt=lineplot.col.opt,alphacol=0.3,col_vec=col_vec,
              ...
              for(i in 1:length(var_sum_list))
              {
               ...
               # var_sum_mat<-ldply(var_sum_list,rbind)
                ...
                write.table(var_sum_mat,file="level1_variable_analysis.txt",sep="\t",row.names=FALSE)
            ...
            if(networktype=="complete"){
              ...
              if(networktype=="GGM"){
                mwan_fdr<-get_partial_cornet(data_matrix, sigfeats.index=sigfeats_index,targeted.index=NA,networkscope="global",
                                             cor.method,abs.cor.thresh,cor.fdrthresh,outloc=output_dir,net_node_colors,net_legend)
              }else{
