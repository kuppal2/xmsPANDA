...
function(X,Y,feature_table_file,parentoutput_dir,class_labels_file,sample.col.opt="rainbow",plots.width=2000,plots.height=2000,plots.res=300, alphacol=0.3,col_vec=NA,pairedanalysis=FALSE,pca.cex.val=4,legendlocation="topright",pca.ellipse=TRUE,ellipse.conf.level=0.95,filename="all",paireddesign=NA,error.bar=TRUE,lineplot.col.opt="black",lineplot.lty.option=c("solid", "dashed", "dotted", "dotdash", "longdash", "twodash"),newdevice=FALSE,timeseries.lineplots=FALSE,alphabetical.order=FALSE,pcascale=TRUE,pcacenter=TRUE,study.design="oneway",lme.modeltype="RI",cex.plots=0.8,ypos.adj.factor=0.5,...)
{
    ...
    ##save(class_labels_levels,file="pcaclass_labels_levels.Rda")
        ...
        #if(pairedanalysis==TRUE)
        {
            if(dim(classlabels_orig)[2]>2){
                ...
                lapply(1:pcnum_limit,function(pc)
                {
                    ...
                    # save(df.summary,shape_vec1,df_time,col_vec,classlabels,classgroup,classlabels_orig,unique_class_col_vec,class_col_vec,ymin,ymax,pca.cex.val,df_time,file=pca_plot_fname)
                    ...
                    if(pairedanalysis==TRUE){
                           ...
                           shape_vec1<-as.numeric(as.character(shape_vec1))
                            ...
                            plot_res<-suppressWarnings(ggplot(df.summary, aes(x = as.factor(x), y = PCscore,color = as.factor(Class),linetype=Class)) + labs(color = "Class",linetype="Class") + geom_point(size = pca.cex.val,shape=shape_vec1) + geom_line(aes(group =as.factor(Class)),size=sizeval) + geom_errorbar(aes(ymin = ymin, ymax = ymax),size=1,width=0.1) + xlab("TimePoint") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                            panel.grid.minor = element_blank(),
                            axis.line = element_line(colour = "black",size=sizeval),
                            axis.text= element_text(size=11*cex.plots), axis.title=element_text(size=14*cex.plots,face="bold"),
                ...
                #save(classgroup,class_levels,classlabels_orig,res,pairedanalysis,col_vec,pc_pval_vec,file="debuglpA.Rda")
                lapply(1:pcnum_limit,function(pc)
                {
                    ylabel_text<-paste("PC",pc,"score",sep="")
		     ...
		     df=as.data.frame(df)
                    ...
                    
                    {
                        if(pairedanalysis==FALSE){
                            ...
                            if(timeseries.lineplots==TRUE){
                                ...
                                panel.grid.minor = element_blank(), axis.line = element_line(colour = "black",size=sizeval),
                                axis.text= element_text(size=12*cex.plots), axis.title=element_text(size=14*cex.plots,face="bold"),
                                strip.background=element_rect(colour="#f0f0f0",fill="#f0f0f0"),
                                strip.text = element_text(face="bold")) + scale_y_continuous(breaks = scales::pretty_breaks(n = 10)))
			   ...
			   #,legend.position="none",
                         ...
                         #   save(df.summary,Class,pca.cex.val,ymin,ymax,class_col_vec,class_levels,classgroup,file="t1.Rda")
                            sizeval=1.2
