...
function(data_m_fc_withfeats, sigfeats.index=NA,targeted.index=NA,networkscope="global",cor.method="spearman",abs.cor.thresh=0.4,
                             cor.fdrthresh=0.2,outloc,net_node_colors,net_legend=FALSE,netrandseed=555,pcor.method="pcor.shrink"){
  ...
  if(FALSE){
    if(is.na(cor.fdrthresh)==FALSE){
      if(cor.fdrthresh>(-1)){
        ...
        fname<-paste("Tables/significant_correlations_",networkscope,"feats_corrsig",cor.fdrthresh,".txt",sep="")
