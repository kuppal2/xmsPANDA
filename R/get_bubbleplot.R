get_bubbleplot <-
function(Xmat, GroupBy=FALSE,xlab.name="X",ylab.name="Y",cex.plots=0.8,newdevice=TRUE,filename="bubbleplot",file.format="pdf",plot.width=8,plot.height=8,plot.res=300,color.scale=c("blue","red")){
  options(warn=-1)
  if(GroupBy==TRUE){
  ...
  p=p+scale_fill_gradient2(low=color.scale[1],mid="white",high=color.scale[2])+scale_size(range=c(floor(min(abs(Xmat$Statistic),na.rm=TRUE)),ceiling(max(abs(Xmat$Statistic),na.rm=TRUE))))
