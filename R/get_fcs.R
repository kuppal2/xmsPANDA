...
function(target.data,target.data.annot=NA,kegg_species_code="hsa",database="pathway",
  ...
  if(length(res)<1){
    ...
    if(length(res)>0){
      ...
      res<-res[,c("SetID","Z.score","MaxMean","Total.Size","Num.Hits","pval.Z.score","pval.MaxMean","pval.meta","FDR.meta","SetName","XID")]
      
      colnames(res)<-c("SetID","Z.stat","MaxMean.stat","Total.Size","Num.Hits","p.Z.stat","p.MaxMean","p.meta","FDR.meta","SetName","XID")
