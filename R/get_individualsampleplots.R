...
function(feature_table_file,class_labels_file,X=NA,Y=NA,parentoutput_dir,newdevice=FALSE,ylabel="Intensity",bar.colors=NA,cex.plots=0.75,sample.col.opt=c("grey57"),plottype="barplot",alphabetical.order=FALSE,name=NA){
<<<<<<< HEAD
    ...
    if(plottype=="barplot"){
      ...
      if(plottype=="point"){
        ...
        if(plottype=="point_grouped"){
          ...
          p=p+geom_point(size=pca.cex.val)+ggtitle(mzlabel_cur) #,aes(colour=barplot.col.opt))
        }
      }
    }
