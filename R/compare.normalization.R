...
normalization.method=c("all","log2quantilenorm","log2transform","znormtransform","lowess_norm","quantile_norm","rangescaling",
                       "paretoscaling","mstus","eigenms_norm","vsn_norm","sva_norm","tic_norm","cubicspline_norm"),input.intensity.scale="raw",
...
study.design=c("multiclass","onewayanova","twowayanova","onewayanovarepeat","twowayanovarepeat"),...){
	...
	normalization.method=tolower(normalization.method)
        ...
        if(normalization.method[1]==c("all")){
            ...
            normalization.method=c("log2quantilenorm","log2transform","znormtransform","lowess_norm","quantile_norm","rangescaling","paretoscaling","mstus","eigenms_norm","vsn_norm","sva_norm","tic_norm","cubicspline_norm")
        ...
        valid_norm_options<-c("log2quantilenorm","log2transform","znormtransform","lowess_norm","quantile_norm","rangescaling","paretoscaling","mstus","eigenms_norm","vsn_norm","sva_norm","none","tic_norm","cubicspline_norm")
	...
	if(check_xlabel>0){
       ...
       data_matrix_list<-lapply(1:length(normalization.method),function(i)
       {
           ...
           if(normalization.method[i]%in%valid_norm_options){
               ...
               dir.create(normalization.method[i])
          ...
          data_matrix_list<-try(data_preprocess(Xmat=Xmat,Ymat=Ymat,feature_table_file=feature_table_file,parentoutput_dir=outloc,class_labels_file=class_labels_file,num_replicates=num_replicates,feat.filt.thresh=NA,summarize.replicates=summarize.replicates,
	...
	summary.na.replacement=summary.na.replacement,featselmethod=featselmethod,pairedanalysis=pairedanalysis,normalization.method=normalization.method[i],input.intensity.scale=input.intensity.scale),silent=TRUE)
       ...
       pcares<-get_pcascoredistplots(X=X,Y=Y,feature_table_file=NA,parentoutput_dir=outloc,class_labels_file=NA,sample.col.opt="journal",
          ...
          num_nodes=2,alphabetical.order=alphabetical.order,pairedanalysis=pairedanalysis,cutree.method=cutree.method,study.design=analysistype)
