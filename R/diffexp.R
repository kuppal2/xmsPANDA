...
function(Xmat=NA,Ymat=NA,feature_table_file,parentoutput_dir=NA,class_labels_file,num_replicates=1,summarize.replicates=TRUE,summary.method="mean",
                  ...
                  hca_type="two-way",pls_vip_thresh=2,num_nodes=2,max_varsel=100,
                  ...
                  optselect=TRUE,max_comp_sel=3,saveRda=FALSE,legendlocation="topleft",pcacenter=TRUE,pcascale=TRUE,pca.cex.val=6,
  ...
  if(length(featselmethod)>1){
    ...
    for(i in 1:length(featselmethod))
    {
     ...
     if(FALSE){
       suppressWarnings(diffexp.res[[i]]<-try(diffexp.child(Xmat,Ymat,feature_table_file,parentoutput_dir,class_labels_file,num_replicates,feat.filt.thresh,summarize.replicates,summary.method,summary.na.replacement,missing.val,rep.max.missing.thresh,
                                                           ...
                                                           alpha.col=alpha.col,similarity.matrix=similarity.matrix,
                                                           outlier.method=outlier.method[1],removeRda=removeRda,color.palette=color.palette,
                                                           plot_DiNa_graph=plot_DiNa_graph,limma.contrasts.type=limma.contrasts.type,plot.boxplots.raw=plot.boxplots.raw,ggplot.type1=ggplot.type1)
        ...
        if(nrow(Xmat)>2){
         ...
         # clusterExport(cl,"svm",envir = .GlobalEnv)
          ...
          g1<-try(get_hca(feature_table_file=NA,parentoutput_dir=outloc,class_labels_file=NA,X=Xmat,Y=Ymat,heatmap.col.opt=heatmap.col.opt,cor.method=cor.method,is.data.znorm=FALSE,analysismode=analysismode,
                      ...
                      input.type="intensity",mainlab="",cexRow=0.4,cexCol=0.4,alphabetical.order=alphabetical.order,study.design=analysistype,similarity.matrix=similarity.matrix),silent=TRUE)
   ...
   # suppressWarnings(
      diffexp.res<-diffexp.child(Xmat,Ymat,feature_table_file,parentoutput_dir,class_labels_file,num_replicates,feat.filt.thresh,summarize.replicates,summary.method,summary.na.replacement,missing.val,rep.max.missing.thresh,
                                 ...
                                 vcovHC.type=vcovHC.type,ggplot.type1=ggplot.type1,facet.nrow=facet.nrow) #,silent=TRUE)
    #)
