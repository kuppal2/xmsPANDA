...
<<<<<<< HEAD
                             ...
                             extra_text=NA,group_by_mat=NA,position_dodge_width=0.75,numnodes=2,...)
{
                ...
                p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Class,fill=Class)) + labs(title=mzname)
                          ...
                          if(is.na(ggplot.type1)==FALSE){
                           if(ggplot.type1==TRUE){
                              ...
                              p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Factor1,fill=Factor1)) +labs(title=mzname) + facet_wrap(~Factor2, scale="free_x")  
                            }else{
                              
                              p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Factor2,fill=Factor2)) +labs(title=mzname) + facet_wrap(~Factor1, scale="free_x")  
                              
                              
                            }
                          }else{
                            p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Factor1,fill=Factor1)) +labs(title=mzname)
                          }
                              ...
                              if(ggplot.type1==TRUE){
                                ...
                                p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Factor1,fill=GroupBy)) +labs(title=mzname) + facet_wrap(~Factor2, scale="free_x")  
                                
                              }else{
                                 
                                p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Factor2,fill=GroupBy)) +labs(title=mzname) + facet_wrap(~Factor1, scale="free_x")  
        ...
        #         p<-p+geom_line(aes(y  = as.numeric(Feature), x = Factor1)) #, group = SubjectID))
            ...
            #  p=p+geom_line(aes(group=SubjectID))
              ...
              p<-p+ labs(y=ylabel) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                                        ...
                                                        panel.spacing=unit(1,"lines"),
                                                        axis.line = element_line(colour = "black",size=1),
                                                        axis.text= element_text(size=14*cex.plots),
                                                        axis.title=element_text(size=18*cex.plots,face="bold"),
                                                        plot.title = element_text(hjust = 0.5,size=18*cex.plots),
...
=======
alphabetical.order=FALSE,name=NA,add.jitter=TRUE,add.pvalues=TRUE,class.levels=NA,fill.plots=FALSE,connectpairedsamples=FALSE,boxplot.type="ggplot",study.design=c("multiclass","onewayanova","twowayanova","onewayanovarepeat","twowayanovarepeat"),...)
{
	   	...
	   	if(check_names==1){
			...
			if(length(class_labels_levels)>=2)
			{
				if(length(class_labels_levels)>=1)
				{
					...
					temp_dm<-cbind(as.character(class_vec),as.vector(feat_vec))
					
					temp_dm2<-temp_dm #[,c(1,(m+1))]
					temp_dm2<-as.data.frame(temp_dm2)
					colnames(temp_dm2)<-c("Class","Feature")
					temp_dm2$Feature<-as.numeric(as.character(temp_dm2$Feature))
                    ...
                    if(multiple.groups==TRUE){
                        Factor1<-gsub(Class,pattern=":([\\w|\\W])*",replacement="",perl=TRUE)
                        Factor2<-gsub(Class,pattern="([\\w|\\W])*:",replacement="",perl=TRUE)
		    ...
		    if(boxplot.type=="simple"){
				...
				#boxplot(cur_d)
                    ...
                    if(add.jitter==FALSE && add.pvalues==FALSE && fill.plots==TRUE){
                        
                        suppressMessages(library(ggplot2))
                        if(multiple.groups==FALSE){
                                p <- ggplot(temp_dm2, aes(y=as.numeric(Feature),x=Class)) + labs(title=mzname)
                        }else{
                            ...
                            p<-p+ labs(y=ylabel) + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                                panel.grid.minor = element_blank(),
                                                axis.line = element_line(colour = "black",size=1),
                                                axis.text= element_text(size=10*cex), axis.title=element_text(size=14,face="bold"),
...
#  save(plot_res,file="boxplot_plot.Rda")
          if(boxplot.type=="ggplot"){
