                        ...
                        sample.col.opt="rainbow",plots.width=8,plots.height=8,plots.res=600, plots.type="cairo", alphacol=0.3, hca_type,newdevice=FALSE,input.type="intensity",mainlab="",cexRow=1, cexCol=1,plot.bycluster=FALSE,color.rows=FALSE,similarity.matrix="correlation",deepsplit=2,minclustsize=10,mergeCutHeight=0.1,num_nodes=2,alphabetical.order=FALSE,pairedanalysis=FALSE,cutree.method="halfheight",study.design="oneway")
{
  ...
  if(input.type=="intensity"){
    ...
    if(similarity.matrix=="TOM"){
      ...
      write.table(simmat,file="Tables/pairwisecorrelation_samples.txt",sep="\t",row.names=FALSE)
      
      #save(data_m,file="data_m.Rda")
      simmat=WGCNA::cor(t(data_m),method=cor.method,use="pairwise.complete.obs")
      distr_m<-1-simmat
      write.table(simmat,file="Tables/pairwisecorrelation_selectedfeatures.txt",sep="\t",row.names=FALSE)
...
>>>>>>> 4ccdcb99e71707b6d2e6cfcfae418ec4bdb9aae3
        ...
        mycl_samples <-cutreeDynamic(hc,distM= distc_m,deepSplit=deepsplit, minClusterSize=minclustsize, pamRespectsDendro = FALSE, pamStage=TRUE,verbose=0)
        ...
        mycl_metabs <-cutreeDynamic(hr,distM=distr_m,deepSplit=deepsplit, minClusterSize=minclustsize, pamRespectsDendro = FALSE, pamStage=TRUE,verbose=0)
        
        m3=try(mergeCloseModules(t(data_m),colors=mycl_metabs,cutHeight=mergeCutheight),silent=TRUE)
  ...
  if(is(hr,"try-error") || is(hc,"try-error")){
    ...
    if(hca_type=="two-way"){
      ...
      if(col_metabs==TRUE){
        ...
        col_vec3 <- topo.colors(length(unique(mycl_metabs)), alpha=alphacol)
        rowcolors=col_vec3[as.numeric(mycl_metabs)]
      }else{
