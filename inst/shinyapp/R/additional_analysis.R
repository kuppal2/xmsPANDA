...
additional_analysis_page <- fluidPage(
  ...
  # Make the Layout of parameter ----
    tags$div(
      ...
      bsCollapse(open = "Compare normalization methods",
      ...
      column(width=6,selectInput(width="350px","normalization.method","Select normalization methods (click in the box to select more options):",multiple=TRUE,c("all(default)"="all",
                                                                                                                       ...
                                                                                                                       "EigenMS normalizaiton"="eigenms_norm",
                                                                                                                        ...
                                                                                                                        "Surrogate variable analysis"="sva_norm",
                                                                                                                         ...
                                                                                                                         "cubicspline normalization"="cubicspline_norm"),selected=c("log2transform","quantile_norm","log2quantilenorm","znormtransform","mstus","eigenms_norm"))),
      column(width=6,selectInput(width="350px","summary_na_replacement","Choose an imputation method:",c("halffeaturemin","knn","randomforest","QRILC","zeros","halfsamplemin","halfdatamin","none"))),
                    column(width=6,
...
column(width=12,
  ...
  column(width=12,
tags$head(tags$style("#moreoptions3 .modal-dialog{ width:1000px}")),
bsModal("moreoptions3", "View results", "optionbutton3", size = "big",
conditionalPanel(condition = "output.checktable1",

                          uiOutput("downloadbutton"),DTOutput('pathwaytb',width=12)
...
tags$div(tags$h5(style='font-weight:bold',"Description:")),
 tags$p(style='font-weight:normal',"Functional class scoring determines enrichment of functional classes or sets (e.g. pathways, modules, classes) in association with a phenotype based on the p-value, VIP, fold change, and other statistics. In stage one, statistics of individual entities (features, metabolites) within each functional class are combined to assign an aggregated statistic at the functional class level. Only the IDs with unique chemical formula are used for aggregation within each functional class. In case of redundancies, the entry with the highest statistic is used. The software applies three methods for aggregation: 1) mean statistic calculated as sum of the statistic of ; 2) z-score calculated as sqrt(number of entities in a set)*average value of the statistic (based on Irizarry 2009; PMC3134237); 3) max-mean method by Efron and Tibshiranil (Efron, Bradley; Tibshirani, Robert. On testing the significance of sets of genes. Ann. Appl. Stat. 1 (2007), no. 1, 107--129. doi:10.1214/07-AOAS101. https://projecteuclid.org/euclid.aoas/1183143731). For each method, a permutation test at the functional class/set level is used to determine p-values. A meta-pvalue is assigned by aggregating the p-values from permutation tests for each method using the mean p method by Edgington ES (A normal curve method for combining probability values from indepen-dent experiments. Journal of Psychology,1972, 82, 85–89). Benjamini-Hochberg method is used for false discovery correction. The input file can be provided in three formats as described below. For the formats where the input file does not contain database identifiers, please see formats 2 and 3 and the corresponding annotation files as a separate input."),
