...
conditionalPanel(condition = "output.checktable1",
                          ...
                          uiOutput("downloadbutton"),DTOutput('pathwaytb',width=8)
...
tags$div(tags$h5(style='font-weight:bold',"Description:")),
 tags$p(style='font-weight:normal',"Functional class scoring determines enrichment of functional classes or sets (e.g. pathways, modules, classes) in association with a phenotype based on the p-value, VIP, fold change, and other statistics. In stage one, statistics of individual entities (features, metabolites) within each functional class are combined to assign an aggregated statistic at the functional class level. Only the IDs with unique chemical formula are used for aggregation within each functional class. In case of redundancies, the entry with the highest statistic is used. The software applies the max-mean method by Efron and Tibshiranil (Efron, Bradley; Tibshirani, Robert. On testing the significance of sets of genes. Ann. Appl. Stat. 1 (2007), no. 1, 107--129. doi:10.1214/07-AOAS101. https://projecteuclid.org/euclid.aoas/1183143731). A permutation test at the functional class/set level is used to determine p-values. Benjamini-Hochberg method is used for false discovery correction. The input file can be provided in three formats as described below. For the formats where the input file does not contain database identifiers, please see formats 2 and 3 and the corresponding annotation files as a separate input."),
